...
function playCurrentSong() {
    ...
    // Remove 'bg-danger' class from all songs
    for (let i = 0; i < Song.length; i++) {
        const songElement = document.getElementById(`song-${i}`);
        if (songElement) {
            songElement.classList.remove('bg-danger', 'playing');
        }
    }
    ...
    audio.addEventListener('loadedmetadata', () => {
        ...
        // Add 'bg-danger' and 'playing' class to the current song
        const currentSongElement = document.getElementById(`song-${currentIndex}`);
        if (currentSongElement) {
            currentSongElement.classList.add('bg-danger', 'playing');
        }
...
document.querySelector('.progress-bar').addEventListener('click', (e) => {
    ...
    const percentClicked = (e.clientX - progressBarRect.left) / progressBarRect.width;
